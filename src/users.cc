...
static void ViewLine(userinfo *u, int y)
{
    ...
    if (u) {
        V.Print(y, 4, TextColor, "%-16.16s  %-8.8s ", u->user, u->logname);
        for (int i = 0; i < GRMAX; ++i)
            V.Put((u->gmask >> i) & 1 ? "0123456789012345"[i] : '-', TextColor);
        V.Put(' ', TextColor);
...
void Users()
{
    ...
    V.Put(y - 2, 4, "Пользователь      Вход     Группы           Фамилия, Имя, Отчество",
          BorderColor);
