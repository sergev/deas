    ...
    Dialog *dlg =
        (!a || a->anal)
            ? new Dialog(" Новая проводка ", DialogPalette,
                         ...
                         "{Описание} S :50:80",
                         &db, ChooseAccount, &cr, ChooseAccount, &sum, &doll, &info.amount.pcs,
                         &info.descr)
            : new Dialog(" Новая проводка ", DialogPalette,
                         "(++{Куда                     Откуда}"
                         "L:20:40 | { } | L:20:40 ) {}"
                         "(++{Сумма} N:1:1999999999:1 R :0{Рубли} :1{Доллары} ) {}"
                         "{Описание} S :50:80",
                         &db, ChooseAccount, &cr, ChooseAccount, &sum, &doll, &info.descr);
again:
    if (dlg->Run(&V) <= 0) {
        delete dlg;
        return 0;
    ...
    accountinfo *c = 0, *d = 0;
    if (!db || !cr || !(c = FindAccount(cr)) || !(d = FindAccount(db)) || !d->wmask || !c->wmask ||
        !sum || (a && a->anal && !info.amount.pcs)) {
        V.Beep();
        goto again;
    }
    delete dlg;
...
int EditJournalEntry(entryinfo *e)
{
    ...
    Dialog dlg(" Изменение проводки ", DialogPalette,
               ...
               "{Описание} S :50:50 {}"
               "{Владелец} S :16:16 {}",
               &db, ChooseAccount, &cr, ChooseAccount, &sum, &doll, &info.amount.pcs, &day, &mon,
               &year, info.descr, info.user);
again:
    if (dlg.Run(&V) <= 0)
        return 0;
...
int DeleteJournalEntry(entryinfo *e)
{
    ...
    if (V.Popup(" Удаление проводки ", buf, name, " Нет ", " Да ", 0, DialogColor,
                DialogOptionColor) != 1)
        return 0;
...
static void ViewJournalLine(int lnum, int y)
{
    ...
    V.Print(y, 2, TextAltColor, "%02d.%02d.%02d  %d", day, mon, year % 100, e->eid);
    if (e->deleted)
        V.Put('#', TextAltColor);
    ...
    if (a)
        V.Print(TextColor, "%c %s", a->deleted ? '#' : IsParent(a) ? '/' : ' ', a->descr);
