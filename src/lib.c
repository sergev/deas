...
static char *getstring(char *str, int len, char *input)
{
    ...
    *str = 0;
    ...
    if (*input == '"')
        lim = 1, ++input;
    while (*input && (!lim || *input != '"') && (lim || !ISSPACE(*input)) && len-- > 0)
        *str++ = *input++;
    if (lim && *input == '"')
        ++input;
    if (input <= sav)
...
static int cinit()
{
    ...
    for (naccounts = 0; naccounts < MAXACCOUNTS; ++naccounts) {
        if (crypt_read(chartfd, &acntab[naccounts].info, sizeof(accountinfo)) !=
            sizeof(accountinfo))
            break;
...
static int uinit()
{
    ...
    fclose(fd);
    if (!*(long *)deas_E || !*(long *)deas_D || !*(long *)deas_P || !*(long *)deas_Q ||
        !*(long *)deas_U || !*(long *)deas_N)
        return 0;
    return 1;
}
...
static int jinit()
{
    ...
    for (i = 0; i < jlen; ++i) {
        if (crypt_read(journfd, journal + i, sizeof(entryinfo)) != sizeof(entryinfo))
            goto failed;
        jindex[i] = i + 1;
    }
    if (jlen)
...
static void adelete(account_t *t, entryinfo *e)
{
    ...
    if (i >= t->nent)
        return;
    if (i < t->nent - 1)
        memmove(t->entry + i, t->entry + i + 1, (t->nent - i - 1) * sizeof(long));
    --t->nent;
    ainvalidate(t);
}
...
static void acompute(account_t *t)
{
    ...
    for (i = 0; i < t->nent; ++i) {
        ...
        if (t->info.acn == e->debit) {
            vadd(&t->total.debit, e->amount.rub, e->amount.doll, e->amount.pcs);
            if (m >= 0 && m < 12)
                vadd(&t->monthly[m].debit, e->amount.rub, e->amount.doll, e->amount.pcs);
        } else if (t->info.acn == e->credit) {
            vadd(&t->total.credit, e->amount.rub, e->amount.doll, e->amount.pcs);
            if (m >= 0 && m < 12)
                vadd(&t->monthly[m].credit, e->amount.rub, e->amount.doll, e->amount.pcs);
        }
    }

    /* Добавляем балансы подсчетов. */
    for (s = acntab; s < acntab + naccounts; ++s) {
        if (s->info.deleted || !s->info.acn || s->info.pacn != t->info.acn)
            continue;
        if (!s->tvalid)
            acompute(s);
...
menuinfo *menu_next(clientinfo *c)
{
    ...
    for (; c->midx < nmenu; ++c->midx) {
        ...
        if (MANAGER(&c->u->info))
            break;
        if (!(cr->wmask & c->u->info.gmask) && !awritable(cr->pacn, c->u->info.gmask))
            continue;
        if (!(db->wmask & c->u->info.gmask) && !awritable(db->pacn, c->u->info.gmask))
            continue;
        break;
    }
    if (c->midx >= nmenu)
...
void journal_add(clientinfo *c, entryinfo *i)
{
    ...
    if (!db || !(c->u->info.gmask & db->info.wmask) || !cr || !(c->u->info.gmask & cr->info.wmask))
        return;
    ...
    if (jlen >= jsz) {
        jsz += 64;
        journal = (entryinfo *)realloc(journal, jsz * sizeof(entryinfo));
...
void journal_edit(clientinfo *c, entryinfo *i)
{
    ...
    if (e->amount.rub != i->amount.rub || e->amount.doll != i->amount.doll ||
