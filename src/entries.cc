...
void LoadEntries()
{
    ...
    for (menuinfo *a = menu_first(&clnt); a; a = menu_next(&clnt)) {
        if (nmenu >= menutabsz) {
            if (!menutabsz) {
                menutabsz += 16;
                menutab = (menuinfo *)malloc(menutabsz * sizeof(*menutab));
            } else {
                menutabsz += 16;
                menutab = (menuinfo *)realloc(menutab, menutabsz * sizeof(*menutab));
            }
            if (!menutab) {
            ...
            if (pd->anal || pc->anal) {
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, &cr, ChooseSubAccount, &sum, &doll,
                                     &info.amount.pcs, &info.descr);
                    ...
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, &cr, ChooseSubAccount, &sum,
                                     &info.amount.pcs, &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                ...
                if (m->currency != 1 && m->currency != 2)
                    ...
                    dlg =
                        new Dialog(title, DialogPalette,
                                   "(++{Куда                     Откуда}"
                                   "L:20:40 | {   } | L:20:40 ) {}"
                                   "(++{Сумма - рубли} N:1:1999999999:1) {}"
                                   "{Описание} S :50:80",
                                   &db, ChooseSubAccount, &cr, ChooseSubAccount, &sum, &info.descr);
                else
                    dlg =
                        new Dialog(title, DialogPalette,
                                   "(++{Куда                     Откуда}"
            ...
            if (pd->anal || pc->anal) {
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, crbuf + 1, &sum, &doll,
                                     &info.amount.pcs, &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, crbuf + 1, &sum, &info.amount.pcs,
                                     &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                ...
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "(++{Сумма} N:1:1999999999:1 R :0{Рубли} :1{Доллары}) {}"
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, crbuf + 1, &sum, &doll, &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "L:20:40 | {   } | T:20 ) {}"
                                     "(++{Сумма - рубли} N:1:1999999999:1) {}"
                                     "{Описание} S :50:80",
                                     &db, ChooseSubAccount, crbuf + 1, &sum, &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "L:20:40 | {   } | T:20 ) {}"
            ...
            if (pd->anal || pc->anal) {
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, &cr, ChooseSubAccount, &sum, &doll,
                                     &info.amount.pcs, &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, &cr, ChooseSubAccount, &sum, &info.amount.pcs,
                                     &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                ...
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "(++{Сумма} N:1:1999999999:1 R :0{Рубли} :1{Доллары}) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, &cr, ChooseSubAccount, &sum, &doll, &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "T:20 | {   } | L:20:40 ) {}"
                                     "(++{Сумма - рубли} N:1:1999999999:1) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, &cr, ChooseSubAccount, &sum, &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "T:20 | {   } | L:20:40 ) {}"
            ...
            if (pd->anal || pc->anal) {
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, crbuf + 1, &sum, &doll, &info.amount.pcs,
                                     &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     ...
                                     "++++++++++++++{Штуки} N:1:99999999:1 ) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, crbuf + 1, &sum, &info.amount.pcs, &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "T:20 | {   } | T:20 ) {}"
                ...
                if (m->currency != 1 && m->currency != 2)
                    dlg = new Dialog(title, DialogPalette,
                                     ...
                                     "(++{Сумма} N:1:1999999999:1 R :0{Рубли} :1{Доллары}) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, crbuf + 1, &sum, &doll, &info.descr);
                else if (m->currency == 1)
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "T:20 | {   } | T:20 ) {}"
                                     "(++{Сумма - рубли} N:1:1999999999:1) {}"
                                     "{Описание} S :50:80",
                                     dbbuf + 1, crbuf + 1, &sum, &info.descr);
                else
                    dlg = new Dialog(title, DialogPalette,
                                     "(++{Куда                     Откуда}"
                                     "T:20 | {   } | T:20 ) {}"
