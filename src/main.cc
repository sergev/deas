...
struct {
    ...
    const char *help;
    int r, w, h;
} menu[] = { { "Операции", Entries, 1, "Проводка операций по счетам\n" },
             { "Счета", Accounts, 1, "Просмотр счетов, балансов и\nпроводок по счетам" },
             { "Журнал", Journal, 1, "Просмотр журнала проводок" },
...
unsigned char DialogPalette[9] = { 0x1f, 0x1f, 0x1e, 0x30, 0x3b, 0x70, 0x74, 0x7f, 0x7e };
...
void PromptQuit()
{
    ...
    V.DelHotKey(alt('q'));
    V.DelHotKey(cntrl('Q'));
    int choice = V.Popup(" Выход ", "Вы действительно хотите выйти?", 0, " Да ", " Нет ", 0,
...
void Fatal(char *msg)
{
    V.Error(ErrorColor, TextColor, " Ошибка ", " Готово ", "Ошибка базы: %s", msg);
    Quit();
}
...
int main(int argc, char **argv)
{
    ...
    Message("Учетная система Кроникс, Версия %s, Copyright (C) 1996 Кроникс", GMAS_VERSION);
    ...
    Flash *alert = new Flash(&V, "", "Подсоединение к серверу...", InverseLightTextColor);
    if (!deas_init(host, 0, Fatal)) {
        delete alert;
        V.Error(ErrorColor, TextColor, " Ошибка ", " Готово ", "Ошибка при инициализации базы");
        Quit();
    }
    delete alert;
    ...
    again:
        ...
        if (dlg->Run(&V) <= 0 || !*username || !*password)
            goto again;
        Flash *alert = new Flash(&V, "", "Проверка полномочий...", InverseLightTextColor);
        clnt.u = user_register(username, password, msec(1));
        delete alert;
        if (clnt.u)
            break;
        V.Error(ErrorColor, TextColor, " Ошибка ", " Готово ", "Неверное имя пользователя");
    }
     ...
     */
    alert = new Flash(&V, "", "Загрузка счетов...", InverseLightTextColor);
    LoadAccounts();
    delete alert;
     ...
     */
    alert = new Flash(&V, "", "Загрузка проводок...", InverseLightTextColor);
    LoadEntries();
    delete alert;
    ...
    int ch = 0;
    PutMultistring(V.Lines / 2 - 2, V.Columns / 2 + 4, V.Columns / 2 - 4, menu[ch].help, HelpColor);
    for (;;) {
        for (int i = 0; i < nent; ++i) {
            int color = (i == ch) ? ActiveMenuColor : MenuColor;
            V.Clear(menu[i].r, c, menu[i].h, w, color, ' ');
            if (menu[i].big) {
                V.Put(menu[i].r + 1, c + (w - menu[i].w) / 2, menu[i].button, color);
                if (i == ch)
                    V.DrawFrame(menu[i].r, c, 3, w, MenuBorderColor);
            } else {
                V.Put(menu[i].r, c + (w - menu[i].w) / 2, menu[i].button, color);
                if (i == ch) {
        ...
        V.Clear(2, V.Columns / 2 + 2, V.Lines - 4, V.Columns / 2 - 4, TextColor, ' ');
        PutMultistring(V.Lines / 2 - 2, V.Columns / 2 + 4, V.Columns / 2 - 4, menu[ch].help,
                       HelpColor);
    }
}
