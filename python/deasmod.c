...
static PyObject *deas_reguser(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|sssl:deas.register", &user, &password, &host, &port))
        return 0;
    if (!host || !*host) {
...
static PyObject *deas_chart_first(PyObject *self, PyObject *args)
{
    ...
    a = chart_first(&clnt);
    if (a)
        return Py_BuildValue("(llllslll)", a->acn, a->pacn, a->passive, a->anal, a->descr,
...
static PyObject *deas_chart_add(PyObject *self, PyObject *args)
{
    ...
    if (!clnt.u || !PyArg_ParseTuple(args, "llllslll:deas.chart_add", &a->acn, &a->pacn,
                                     &a->passive, &a->anal, &dp, &a->deleted, &a->rmask, &a->wmask))
        return 0;
...
static PyObject *deas_chart_edit(PyObject *self, PyObject *args)
{
    ...
    if (!clnt.u ||
        !PyArg_ParseTuple(args, "llllslll:deas.chart_edit", &a->acn, &a->pacn, &a->passive,
                          &a->anal, &dp, a->descr, &a->deleted, &a->rmask, &a->wmask))
        return 0;
    strncpy(a->descr, dp, sizeof(a->descr) - 1);
    chart_edit(&clnt, a);
    Py_INCREF(Py_None);
...
static PyObject *deas_account_first(PyObject *self, PyObject *args)
{
    ...
    if (e)
        return Py_BuildValue("(llll(llll)slls)", e->eid, e->date, e->debit, e->credit,
                             e->amount.rub, e->amount.doll, e->amount.pcs, e->amount.rec, e->descr,
                             e->deleted, e->rmask, e->user);
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *deas_account_balance(PyObject *self, PyObject *args)
{
    ...
    if (!b)
        return 0;
    return Py_BuildValue("((llll)(llll))", b->debit.rub, b->debit.doll, b->debit.pcs, b->debit.rec,
...
static PyObject *deas_account_report(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue(
        ...
        "(llll)(llll),(llll)(llll),(llll)(llll),(llll)(llll),"
        "(llll)(llll)]",
        b[0].debit.rub, b[0].debit.doll, b[0].debit.pcs, b[0].debit.rec, b[0].credit.rub,
        b[0].credit.doll, b[0].credit.pcs, b[0].credit.rec, b[1].debit.rub, b[1].debit.doll,
        b[1].debit.pcs, b[1].debit.rec, b[1].credit.rub, b[1].credit.doll, b[1].credit.pcs,
        b[1].credit.rec, b[2].debit.rub, b[2].debit.doll, b[2].debit.pcs, b[2].debit.rec,
        b[2].credit.rub, b[2].credit.doll, b[2].credit.pcs, b[2].credit.rec, b[3].debit.rub,
        b[3].debit.doll, b[3].debit.pcs, b[3].debit.rec, b[3].credit.rub, b[3].credit.doll,
        b[3].credit.pcs, b[3].credit.rec, b[4].debit.rub, b[4].debit.doll, b[4].debit.pcs,
        b[4].debit.rec, b[4].credit.rub, b[4].credit.doll, b[4].credit.pcs, b[4].credit.rec,
        b[5].debit.rub, b[5].debit.doll, b[5].debit.pcs, b[5].debit.rec, b[5].credit.rub,
        b[5].credit.doll, b[5].credit.pcs, b[5].credit.rec, b[6].debit.rub, b[6].debit.doll,
        b[6].debit.pcs, b[6].debit.rec, b[6].credit.rub, b[6].credit.doll, b[6].credit.pcs,
        b[6].credit.rec, b[7].debit.rub, b[7].debit.doll, b[7].debit.pcs, b[7].debit.rec,
        b[7].credit.rub, b[7].credit.doll, b[7].credit.pcs, b[7].credit.rec, b[8].debit.rub,
        b[8].debit.doll, b[8].debit.pcs, b[8].debit.rec, b[8].credit.rub, b[8].credit.doll,
        b[8].credit.pcs, b[8].credit.rec, b[9].debit.rub, b[9].debit.doll, b[9].debit.pcs,
        b[9].debit.rec, b[9].credit.rub, b[9].credit.doll, b[9].credit.pcs, b[9].credit.rec,
        b[10].debit.rub, b[10].debit.doll, b[10].debit.pcs, b[10].debit.rec, b[10].credit.rub,
        b[10].credit.doll, b[10].credit.pcs, b[10].credit.rec, b[11].debit.rub, b[11].debit.doll,
        b[11].debit.pcs, b[11].debit.rec, b[11].credit.rub, b[11].credit.doll, b[11].credit.pcs,
        b[11].credit.rec);
}
...
static PyObject *deas_journal_add(PyObject *self, PyObject *args)
{
    ...
    if (!clnt.u ||
        !PyArg_ParseTuple(args, "llll(llll)slls:deas.journal_add", &e->eid, &e->date, &e->debit,
                          &e->credit, &e->amount.rub, &e->amount.doll, &e->amount.pcs,
                          &e->amount.rec, &dp, &e->deleted, &e->rmask, &up))
        return 0;
...
static PyObject *deas_journal_edit(PyObject *self, PyObject *args)
{
    ...
    if (!clnt.u ||
        !PyArg_ParseTuple(args, "llll(llll)slls:deas.journal_edit", &e->eid, &e->date, &e->debit,
                          &e->credit, &e->amount.rub, &e->amount.doll, &e->amount.pcs,
                          &e->amount.rec, &dp, &e->deleted, &e->rmask, &up))
        return 0;
    strncpy(e->descr, dp, sizeof(e->descr) - 1);
...
static PyObject *deas_menu_first(PyObject *self, PyObject *args)
{
    ...
    m = menu_first(&clnt);
    if (m)
        return Py_BuildValue("(lllss)", m->debit, m->credit, m->currency, m->line, m->descr);
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *deas_user_first(PyObject *self, PyObject *args)
{
    ...
    u = user_first(&clnt);
    if (u)
        return Py_BuildValue("(sssl)", u->logname, u->user, u->descr, u->gmask);
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *deas_ymd_to_date(PyObject *self, PyObject *args)
{
    ...
    if (!clnt.u || !PyArg_ParseTuple(args, "iii:deas.ymd_to_date", &y, &m, &d))
        return 0;
...
static PyMethodDef deas_methods[] = { { "register", deas_reguser, 1 },
