...
static void puthexw16(word16 w); /* Put out 16-bit word in hex,
                                    high byte first. */
static void putstr(string s);    /* Put out null-terminated ASCII
                                    string via putchar. */
...
int str2reg(unitptr reg, string digitstr)
{
    ...
    while (i > 0 && (digitstr[i - 1] == ' ' || digitstr[i - 1] == '\n'))
        digitstr[--i] = 0;
 ...
 */
{
    ...
    commaplaces = (radix == 10 ? 3 : (radix == 16 ? 4 : (radix == 2 ? 8 : (radix == 8 ? 8 : 1))));
    mp_move(r, n);
