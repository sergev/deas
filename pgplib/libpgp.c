...
//	void *E, void *N);
int rsa_private_encrypt(void *outbuf, void *inbuf, short bytes, void *E, void *D, void *P, void *Q,
                        void *U, void *N);
int rsa_public_decrypt(void *outbuf, void *inbuf, void *E, void *N);
 ...
 */
short readkeypacket(FILE *f, struct IdeaCfbContext *cfb, byte *pctb, char *userid, void *n, void *e,
                    void *d, void *p, void *q, void *u, byte *sigkeyID, byte *keyctrl)
{
    byte ctb;
    word16 cert_length;
    int count;
    ...
    if ((ctb != CTB_CERT_PUBKEY) && (ctb != CTB_CERT_SECKEY) && (ctb != CTB_USERID) &&
        (ctb != CTB_KEYCTRL) && !is_ctb_type(ctb, CTB_SKE_TYPE) &&
...
#define SKIP_RETURN(x) return fseek(f, next_packet, SEEK_SET), x
    ...
    if (is_ctb_type(ctb, CTB_CERT_SECKEY_TYPE)) {
        ...
        cert_length -= 1 + (countbytes(d) + 2) + (countbytes(p) + 2) + (countbytes(q) + 2) +
                       (countbytes(u) + 2) + 2;
 ...
 */
int getpublickey(char *keyfile, long *_file_position, char *userid, void *n, void *e)
{
 ...
 */
int getsecretkey(unsigned char *ideakey, char *keyfile, char *userid, void *n, void *e, void *d,
...
int private_decrypt(void *fp, void *tp, int len, char *E, char *D, char *P, char *Q, char *U,
                    char *N)
{
    char *from = fp, *to = tp;
    int n, outlen, q;
