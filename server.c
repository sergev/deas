...
account_reply *chart_first_1 (void *voidarg, clientinfo *clnt)
{
	...
	if (r) result.info = *r; else result.error = 1;
	return &result;
}
...
report_reply *account_report_1 (long *acn, clientinfo *clnt)
{
	...
	memset (&result, 0, sizeof (result));
	if (r) memcpy (&result.info, r, sizeof (result.info));
	else result.error = 1;
	return &result;
}
...
int authenticate (int sock, char *data, int len)
{
	...
	len = private_decrypt (data, &auth, len,
		deas_E, deas_D, deas_P, deas_Q, deas_U, deas_N);
...
failed:         result = 0;
	...
	len = public_decrypt (auth.data, key, sizeof (auth.data),
		user_E (clnt), user_N (clnt));
	if (len != sizeof (key))
		goto failed;
...
closeit:        close (sock);
		...
		if (len < 0 && clnt->u)
			syslog (LOG_NOTICE, "%s/%d - logout",
				clnt->u->info.logname, sock);
		else
			syslog (LOG_NOTICE, "%s - protocol error",
	...
	if (! clnt->u) {
		if (! authenticate (sock, data, len))
			goto closeit;
		syslog (LOG_NOTICE, "%s/%d - login from %s", clnt->u->info.logname,
			sock, inet_ntoa (client.sin_addr));
		return;
	}
	block_decrypt (data, &req, len, clnt->key);
	switch (req.op) {
	case OPTION_DATE:     name = "OPTION_DATE";     argsz = sizeof (long);        resultsz = sizeof (long);          local = (char*(*)()) option_date_1;     break;
	case OPTION_DELETED:  name = "OPTION_DELETED";  argsz = sizeof (long);        resultsz = sizeof (long);          local = (char*(*)()) option_deleted_1;  break;
	case CHART_FIRST:     name = "CHART_FIRST";     argsz = 0;                    resultsz = sizeof (account_reply); local = (char*(*)()) chart_first_1;     break;
	case CHART_NEXT:      name = "CHART_NEXT";      argsz = 0;                    resultsz = sizeof (account_reply); local = (char*(*)()) chart_next_1;      break;
	case CHART_INFO:      name = "CHART_INFO";      argsz = sizeof (long);        resultsz = sizeof (account_reply); local = (char*(*)()) chart_info_1;      break;
	case CHART_ADD:       name = "CHART_ADD";       argsz = sizeof (accountinfo); resultsz = 0;                      local = (char*(*)()) chart_add_1;       break;
	case CHART_EDIT:      name = "CHART_EDIT";      argsz = sizeof (accountinfo); resultsz = 0;                      local = (char*(*)()) chart_edit_1;      break;
	case CHART_DELETE:    name = "CHART_DELETE";    argsz = sizeof (long);        resultsz = 0;                      local = (char*(*)()) chart_delete_1;    break;
	case ACCOUNT_FIRST:   name = "ACCOUNT_FIRST";   argsz = sizeof (long);        resultsz = sizeof (entry_reply);   local = (char*(*)()) account_first_1;   break;
	case ACCOUNT_NEXT:    name = "ACCOUNT_NEXT";    argsz = 0;                    resultsz = sizeof (entry_reply);   local = (char*(*)()) account_next_1;    break;
	case ACCOUNT_BALANCE: name = "ACCOUNT_BALANCE"; argsz = sizeof (long);        resultsz = sizeof (balance_reply); local = (char*(*)()) account_balance_1; break;
	case ACCOUNT_REPORT:  name = "ACCOUNT_REPORT";  argsz = sizeof (long);        resultsz = sizeof (report_reply);  local = (char*(*)()) account_report_1;  break;
	case JOURNAL_FIRST:   name = "JOURNAL_FIRST";   argsz = 0;                    resultsz = sizeof (entry_reply);   local = (char*(*)()) journal_first_1;   break;
	case JOURNAL_NEXT:    name = "JOURNAL_NEXT";    argsz = 0;                    resultsz = sizeof (entry_reply);   local = (char*(*)()) journal_next_1;    break;
	case JOURNAL_INFO:    name = "JOURNAL_INFO";    argsz = sizeof (long);        resultsz = sizeof (entry_reply);   local = (char*(*)()) journal_info_1;    break;
	case JOURNAL_ADD:     name = "JOURNAL_ADD";     argsz = sizeof (entryinfo);   resultsz = 0;                      local = (char*(*)()) journal_add_1;     break;
	case JOURNAL_EDIT:    name = "JOURNAL_EDIT";    argsz = sizeof (entryinfo);   resultsz = 0;                      local = (char*(*)()) journal_edit_1;    break;
	case JOURNAL_DELETE:  name = "JOURNAL_DELETE";  argsz = sizeof (long);        resultsz = 0;                      local = (char*(*)()) journal_delete_1;  break;
	case USER_FIRST:      name = "USER_FIRST";      argsz = 0;                    resultsz = sizeof (user_reply);    local = (char*(*)()) user_first_1;      break;
	case USER_NEXT:       name = "USER_NEXT";       argsz = 0;                    resultsz = sizeof (user_reply);    local = (char*(*)()) user_next_1;       break;
	case MENU_FIRST:      name = "MENU_FIRST";      argsz = 0;                    resultsz = sizeof (menu_reply);    local = (char*(*)()) menu_first_1;      break;
	case MENU_NEXT:       name = "MENU_NEXT";       argsz = 0;                    resultsz = sizeof (menu_reply);    local = (char*(*)()) menu_next_1;       break;
	default:
		if (debug)
...
int main (int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "Dp:d:")) != EOF)
		...
		case 'd':
			...
			break;
		default:
usage:                  fatal ("usage: deasd [-D] [-p port] [-d dir]");
		}
	argc -= optind;
	argv += optind;
	...
	for (;;) {
		...
		if (FD_ISSET (sock, &readfds) &&
		    (sel = accept (sock, (struct sockaddr*)&client, &i)) >= 0) {
			/* Новое соединение */
			if (debug)
			...
			if (sel >= MAXCLIENTS)
				...
				int val = 2;
				setsockopt (sel, SOL_SOCKET, SO_RCVLOWAT, &val,
					sizeof (val));
				setsockopt (sel, IPPROTO_TCP, TCP_NODELAY, &val,
					sizeof (val));
				FD_SET (sel, &openfds);
