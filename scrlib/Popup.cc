...
int Screen::Popup(const char *head, const char *mesg, const char *mesg2, const char *c1,
                  const char *c2, const char *c3, int color, int inverse)
{
    int len;
    int w = strlen(mesg);
    if (mesg2) {
        ...
        for (int i = 0; i < cnum; ++i) {
            if (i == ch) {
                ...
                if (!HasInverse()) {
                    ...
                    Put(tab[i].r, tab[i].c + strlen(tab[i].name) - 1, ']', inverse);
                }
            } else
...
char *Screen::GetString(int w, const char *str, const char *head, const char *mesg, int color,
                        int inverse)
{
    int len = strlen(mesg);
    if (len > w)
...
void Screen::PopupString(const char *title, const char *str, const char *reply, int color,
                         int inverse)
{
    int w, h, r, c;
...
int Screen::SelectFromList(int color, int inverse, char *head, char *mesg, int cnt, ...)
{
    int w = strlen(mesg);
    int len = strlen(head);
    if (len > w)
...
int Screen::SelectFromTable(int color, int inverse, char *head, char *mesg, int cnt, char *elem[])
{
    int w = strlen(mesg);
    int len = strlen(head);
    if (len > w)
