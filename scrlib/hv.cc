...
void PromptQuit()
{
    int choice = V.Popup(" Quit ", "Do you really want to quit ?", 0, " Yes ", " No ", 0,
...
{
    if (!fd) {
        V.Error(ErrorColor, ErrorInverse, "Error", "Ok", "Cannot read %s", filename);
        return;
    }
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            case meta('B'): { // f2 - goto
                ...
                char *p =
                    V.GetString(8, gotobuf, " Goto ", "Go to offset", PopupColor, PopupInverse);
                if (!p)
                    continue;
                unsigned long off;
                sscanf(p, "%lx", &off);
                if (off > flen) {
                    V.Error(ErrorColor, ErrorInverse, "Error", "Ok", "Too big offset %lx", off);
                    continue;
                }
                bn = (off + bsz - 1) / bsz;
                break;
            ...
            case meta('E'): { // f5 - block size
                ...
                auto *p =
                    V.GetString(7, sizebuf, " Block ", "Set block size", PopupColor, PopupInverse);
                if (!p)
                    continue;
                auto off = bn * bsz;
                sscanf(p, "%lx", &bsz);
...
int main(int argc, char **argv)
{
    ...
    if (argc < 1) {
    usage:
        V.Error(ErrorColor, ErrorInverse, "Usage", "Ok", "Usage: hv file [offset]");
        V.Move(V.Lines - 1, 0);
        V.Sync();
        exit(-1);
    }
