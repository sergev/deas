...
DialogLabel::DialogLabel(int row, int col, const char **p) : DialogElement(row, col)
{
    label = GetString(p);
...
DialogButton::DialogButton(int row, int col, const char **p, int *v) : DialogElement(row, col, 1)
{
    label = GetString(p);
...
DialogTitle::DialogTitle(int row, int col, const char **p, char *v) : DialogElement(row, col, 0)
{
    w = GetNumber(p);
    h = 1;
    val = v;
...
DialogString::DialogString(int row, int col, const char **p, char *v) : DialogElement(row, col, 1)
{
    w = GetNumber(p);
    maxlen = GetNumber(p);
    if (!maxlen)
...
DialogNumber::DialogNumber(int row, int col, const char **p, long *v) : DialogElement(row, col, 1)
{
    min = GetNumber(p);
    max = GetNumber(p);
    step = GetNumber(p);
...
DialogMask::DialogMask(int row, int col, const char **p, long *v) : DialogElement(row, col, 1)
{
    w = GetNumber(p);
    h = 1;
    val = v;
...
DialogCommand::DialogCommand(int row, int col, const char **p) : DialogElement(row, col, 1)
{
    val = GetNumber(p);
    label = GetString(p);
    w = label ? strlen(label) : 0;
...
{
    scr->Put(r + baserow, c + basecol, *val ? "[*] " : "[ ] ", palette[ColorTextSelected]);
    if (label)
...
int DialogCommand::Run()
{
    if (label)
        scr->Put(r + baserow, c + basecol, label,
                 val == 1 ? palette[ColorCommandDefaultSelected] : palette[ColorCommandSelected]);
    for (;;) {
        scr->HideCursor();
        scr->Sync();
        switch (scr->GetKey()) {
...
{
    scr->PutLimited(r + baserow, c + basecol, val, w, palette[ColorFieldSelected]);
    int x = 0;
    int len = strlen(val);
    for (;;) {
    ...
    for (;;) {
        ...
        default:
            ...
            if (len < maxlen)
                ++len;
            val[len] = 0;
            scr->Put(r + baserow, c + basecol + len - 1, '*', palette[ColorFieldSelected]);
            break;
        case cntrl('H'): // Backspace
            if (x == 0)
                continue;
        ...
        case meta('x'): // Delete
            ...
            for (int i = x; i < len; ++i)
                val[i] = val[i + 1];
            --len;
            scr->Put(r + baserow, c + basecol + len, ' ', palette[ColorFieldSelected]);
            break;
        }
    }
}
...
DialogGroup::DialogGroup(int row0, int col0, const char **fmt, void ***arg, int flags,
                         DialogGroup *parent)
    : DialogElement(row0, col0)
{
    ...
    while (**fmt) {
        ...
        case '(':
            ...
            break;
        case '[':
            v = new DialogGroup(currow, curcol, fmt, arg, GRP_FRAMED);
            break;
        case '<':
            v = new DialogGroup(currow, curcol, fmt, arg, GRP_CENTER, this);
            break;
        case 'b':
        case 'B':
            v = new DialogButton(currow, curcol, fmt, (int *)*(*arg)++);
            v->oblig = oblig;
            break;
        case 't':
        case 'T':
            v = new DialogTitle(currow, curcol, fmt, (char *)*(*arg)++);
            v->oblig = oblig;
            break;
        case 's':
        case 'S':
            v = new DialogString(currow, curcol, fmt, (char *)*(*arg)++);
            v->oblig = oblig;
            break;
        case 'p':
        case 'P':
            v = new DialogPassword(currow, curcol, fmt, (char *)*(*arg)++);
            v->oblig = oblig;
            break;
        case 'l':
        case 'L':
            arg1 = *(*arg)++;
            v = new DialogList(currow, curcol, fmt, (long *)arg1, (int (*)(...)) * (*arg)++);
            v->oblig = oblig;
            break;
        case 'r':
        case 'R':
            v = new DialogRadioButton(currow, curcol, fmt, (int *)*(*arg)++);
            break;
        case ':':
            --(*fmt);
            v = new DialogRadioButton(currow, curcol, fmt, (int *)(*arg)[-1]);
            break;
        case 'n':
        case 'N':
            v = new DialogNumber(currow, curcol, fmt, (long *)*(*arg)++);
            v->oblig = oblig;
            break;
        case 'm':
        case 'M':
            v = new DialogMask(currow, curcol, fmt, (long *)*(*arg)++);
            v->oblig = oblig;
            break;
        }
        if (v) {
